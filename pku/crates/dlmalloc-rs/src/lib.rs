...
pub use self::domain::{Domain, PKEY_DISABLE_ACCESS, PKEY_DISABLE_WRITE};

#[cfg(target_family = "wasm")]
pub use self::sys::Monitor;
...
// pub struct Dlmalloc<A = System>(nativemalloc::Dlmalloc<A>);
...
impl Dlmalloc<System> {
    ...
    pub const fn new() -> Dlmalloc<System> {
        ...
        // Dlmalloc(nativemalloc::Dlmalloc::new(System::new()))
    }
}
...
impl<A> Dlmalloc<A> {
    ...
    pub const fn new_with_allocator(sys_allocator: A) -> Dlmalloc<A> {
        ...
        // Dlmalloc(nativemalloc::Dlmalloc::new(sys_allocator))
    }
}
