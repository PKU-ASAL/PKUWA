...
use std::str;
use wastlhw::parser::{self, ParseBuffer};
...
fn _parse_str(wat: &str) -> Result<Vec<u8>> {
    ...
    let mut ast = parser::parse::<wastlhw::Wat>(&buf).map_err(|e| Error::cvt(e, wat))?;
...
impl Error {
    fn cvt<E: Into<wastlhw::Error>>(e: E, contents: &str) -> Error {
        let mut err = e.into();
