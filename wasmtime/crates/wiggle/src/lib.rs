use libc::{self, SYS_pkey_alloc, SYS_pkey_free, SYS_pkey_mprotect};
use std::fmt;
use std::slice;
use std::str;
use std::sync::Arc;
...
impl<'a> GuestSliceMut<'a, u8> {
    ...
    fn pkey_free(&self) {
        let pkey = u32::from_be_bytes(
            <[u8; 4]>::try_from(&self.ptr[2..6]).expect("u32::from_be_bytes fails"),
        );
        unsafe {
            if libc::syscall(SYS_pkey_free, pkey) == -1 {
                println!("error in libc::syscall SYS_pkey_free");
            }
        }
    }
    ...
    fn mmap(&mut self) {
        ...
        let prot = self.ptr[10] as i32;
        let flags = self.ptr[11] as i32;
        let map_addr = unsafe {
            libc::mmap(addr as *mut _, len as usize, prot, flags, -1, 0)
        };
        if map_addr == libc::MAP_FAILED {
            println!("libc::mmap fails");
        } else {
            unsafe {
                let ptr = map_addr.sub(self.mem.base().0 as usize);
                let ptr_usize: usize = std::mem::transmute(ptr);
                let ptr_u32 = ptr_usize as u32;
                self.ptr[2..6].copy_from_slice(&ptr_u32.to_be_bytes());
            }
        }
    }
    ...
    pub fn new_wasi_func(&mut self) {
        ...
        let pkfree: &[u8] = &[0x01, 0x4B];
        let pkprotect: &[u8] = &[0x01, 0x20];
        let mmap: &[u8] = &[0x01, 0x2B];
        if &self.ptr[0..3] == rdpkru {
            ...
            self.pkey_alloc();
        } else if &self.ptr[0..2] == pkfree {
            self.pkey_free();
        } else if &self.ptr[0..2] == pkprotect {
            self.pkey_protect();
        } else if &self.ptr[0..2] == mmap {
            self.mmap();
        } else {
