//! The module that implements the `wasmtime call` command.
...
use crate::commands::define_sidecar_function;
...
impl CallCommand {
    /// Splits the command line arguments into ranges of indices.
    fn split_args(&self) -> Result<Vec<Range<usize>>, Error> {
        let mut ranges = Vec::<Range<usize>>::new();

        if self.module_and_args.is_empty() {
            bail!("no module specified to run");
        }
        ...
        for (i, arg) in self.module_and_args.iter().enumerate() {
            // If the argument equals `--module`, then we start a new range.
            if arg == "module:" {
                if in_args {
                    ranges.push(start..i);
                } else {
        ...
        ranges.push(start..self.module_and_args.len());
        Ok(ranges)
    }
    ...
    pub fn execute(mut self) -> Result<()> {
        ...
        let splited_args = self.split_args()?;
        let run_targets: Vec<RunTarget> = splited_args
            .iter()
            .map(|range| {
                let arg = &self.module_and_args[range.start];
                self.run.load_module(&engine, arg.as_ref())
            })
            .collect::<Result<_>>()?;
        ...
        let handles: Vec<_> = run_targets
            ...
            .map(|(index, target)| {
                ...
                thread::spawn(move || {
                    ...
                    if let Some(enable) = self_clone.run.common.wasm.unknown_exports_allow {
                        match &mut linker {
                            CliLinker::Core(l) => {
                    ...
                    match &mut linker {
                        CliLinker::Core(l) => {
                            define_sidecar_function(l);
                        }
                        CliLinker::Component(_) => {
                            bail!("not supported with components");
                        }
                    }

                    let mut store = Store::new(&engine, host);
                    self_clone.populate_with_wasi(
                        &mut linker,
                        &mut store,
                        &target,
                        range.clone(),
                    ...
                    store.data_mut().limits = self_clone.run.store_limits();
                    store.limiter(|t| &mut t.limits);

                    // If fuel has been configured, we want to add the configured
                    // fuel amount to this store.
                    if let Some(fuel) = self_clone.run.common.wasm.fuel {
                        store.set_fuel(fuel)?;
                    }
                    ...
                    let result = runtime.block_on(async {
                        tokio::time::timeout(dur, async {
                            ...
                            if let RunTarget::Core(m) = &target {
                                profiled_modules.push(("".to_string(), m.clone()));
                            }

                            // Load the preload wasm modules.
                            for (name, path) in self_clone.preloads.iter() {
                                // Read the wasm module binary either as `*.wat` or a raw binary
                                let preload_target = self_clone.run.load_module(&engine, path)?;
                            ...
                            self_clone
                                ...
                                .with_context(|| {
                                    format!(
                                        ...
                                        self_clone.module_and_args[0].to_string_lossy()
                                    )
                                })
                        })
                        .await
    ...
    fn compute_argv(&self, range: Range<usize>) -> Result<Vec<String>> {
        let mut result = Vec::new();

        for (i, arg) in self.module_and_args[range.start..range.end]
            .iter()
            .enumerate()
        {
                ...
                CliLinker::Core(linker) => {
                    match (self.run.common.wasi.preview2, self.run.common.wasi.threads) {
                        ...
                        (Some(true), _) | (None, Some(false) | None) => {
                            ...
                            self.set_preview2_ctx(store, range)?;
                        }
                    }
                }
                #[cfg(feature = "component-model")]
    ...
    fn set_preview1_ctx(&self, store: &mut Store<Host>) -> Result<()> {
        ...
        builder
            .inherit_stdio()
            .args(&self.compute_argv(Range { start: 0, end: 1 })?)?;
    ...
    fn set_preview2_ctx(&self, store: &mut Store<Host>, range: Range<usize>) -> Result<()> {
        let mut builder = wasmtime_wasi::p2::WasiCtxBuilder::new();
        builder.inherit_stdio().args(&self.compute_argv(range)?);
