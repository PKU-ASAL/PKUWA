...
typedef void* (*GetMemoryAddress)();
GetMemoryAddress g_func = NULL;

void* g_MmapStart = NULL;
void* g_MmapEnd = NULL;
const uintptr_t g_ConstGrowSize = 65536;

void* LinearMmap(void* start, size_t length, int prot, int flags, int fd, off_t offset)
{
	if(start != NULL)
	{
		return MAP_FAILED;
	}
	if(g_MmapStart == NULL || (uintptr_t)g_MmapStart + length < (uintptr_t)g_MmapEnd)
	{
		if(g_func == NULL)
		{
		...
		{
			g_MmapEnd = (void*)((uintptr_t)g_MmapStart + g_ConstGrowSize);
		}
	}
	void* ret = g_MmapStart;
	g_MmapStart = (void*)((uintptr_t)g_MmapStart + length);
	return ret;
}

int LinearMunmap(void* start, size_t length)
{
	if((uintptr_t)start + length == (uintptr_t)g_MmapStart)
	{
		g_MmapStart = start;
	}
	else
	{
		// TODO: manage the linear memory
		printf("TODO: Cannot unmap linear memory\n");
	}
	return 0;
}
...
struct meta *alloc_meta(void)
{
	...
	if (!ctx.avail_meta_count) {
		...
		if (!ctx.avail_meta_area_count) {
			...
			p = LinearMmap(0, n*pagesize, PROT_NONE,
				MAP_PRIVATE|MAP_ANON, -1, 0);
...
static struct meta *alloc_group(int sc, size_t req)
{
	...
	if (size*cnt+UNIT > pagesize/2) {
		...
		p = LinearMmap(0, needed, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0);
		if (p==MAP_FAILED) {
			free_meta(m);
			return 0;
		}
...
void *malloc(size_t n)
{
	...
	if (n >= MMAP_THRESHOLD) {
		...
		// 	MAP_PRIVATE|MAP_ANON, -1, 0);
		void *p = LinearMmap(0, needed, PROT_READ|PROT_WRITE,
			MAP_PRIVATE|MAP_ANON, -1, 0);
		if (p==MAP_FAILED) return 0;
		wrlock();
		step_seq();
...
void RegisterMemoryRegion(GetMemoryAddress func)
{
	g_func = func;
}
